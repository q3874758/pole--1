...
pub fn create_wallet(data_dir: &PathBuf, comment: Option<String>, password: &str) -> Result<String> {
    let mnemonic = generate_mnemonic();
    let seed_bytes: [u8; 32] = mnemonic
        .to_seed("")
        .as_slice()
        .try_into()
        .map_err(|_| crate::wallet::error::WalletError::Crypto("seed not 32 bytes".to_string()))?;
...
pub fn set_reward_address(data_dir: &PathBuf, config_path: &PathBuf, password: &str) -> Result<String> {
    ...
    let mut config: serde_json::Value = serde_json::from_str(&content)
        .map_err(|e| crate::wallet::error::WalletError::Json(e))?;
