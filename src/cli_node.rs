...
pub const NODE_USAGE_COMMANDS: &[&str] = &[
...
fn print_node_status_summary(summary: &crate::NodeStatusSummary) {
...
fn execute_run_once(
    ...
    let mut progress = LocalNodeProgress::load_or_default(crate::progress_path(&config), &config)?;
    let client = ReqwestHttpTextClient;
...
fn issue_replica_receipt(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("payload_hash={}", crate::hex_32(record.payload_hash));
...
) -> Result<crate::AssembledBatch, Box<dyn std::error::Error>> {
...
fn development_signature_placeholder(
    ...
    crate::stable_hash32(seed.as_bytes()).to_vec()
}
...
mod tests {
    ...
    fn test_config(name: &str) -> NodeConfig {
        NodeConfig {
            ...
            capabilities: crate::CapabilityConfig {
                collect: true,
                store: true,
                verify: true,
                propose: true,
            ...
            runtime: crate::RuntimeConfig {
                ...
                p2p_simulation: crate::P2pSimulationConfig::default(),
                p2p_socket: crate::P2pSocketConfig::default(),
            ...
            reward: crate::RewardConfig::default(),
        }
    }
    ...
    fn run_once_simulates_p2p_propagation() {
        ...
        let mut progress =
            LocalNodeProgress::load_or_default(crate::progress_path(&config), &config).unwrap();
        let execution = run_once_with_client(
            &config,
            &mut progress,
            &client,
            NetworkMode::InMemorySimulation(crate::P2pSimulationConfig::default()),
        )
        .unwrap();
    ...
    fn parse_simulation_topology_args_supports_defaults_and_flags() {
        ...
        let fallback_topology = parse_simulation_topology_args(
            ...
            2,
            "usage",
            crate::P2pSimulationConfig::default(),
        )
        .unwrap();
    ...
    fn run_once_simulates_custom_p2p_topology() {
        ...
        let execution = run_once_with_client(
            ...
            &mut progress,
            &client,
            NetworkMode::InMemorySimulation(crate::P2pSimulationConfig {
                batch_listener_count: 2,
                receipt_listener_count: 1,
    ...
    fn run_once_uses_config_backed_simulation_topology_defaults() {
        ...
        config.runtime.p2p_simulation = crate::P2pSimulationConfig {
            batch_listener_count: 2,
            receipt_listener_count: 1,
            dual_listener_count: 2,
        };
