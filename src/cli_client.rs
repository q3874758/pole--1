...
use crate::{
    ...
    prune_retention, recognition_cache_path, resolve_challenge_window_blocks_arg,
    ...
    ServiceRuntime, ServiceSnapshot, SocketP2pNetwork,
};
type ClientCommandHandler = crate::CommandHandler;
...
fn print_client_node_status(summary: crate::NodeStatusSummary) {
...
fn print_client_network_and_retention_diagnostics(summary: &crate::NodeStatusSummary) {
    println!(
        "configured_p2p_batch_listeners={}",
        summary.configured_p2p_batch_listener_count
    );
...
fn p2p_socket_add_peer_cmd(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = crate::parse_socket_addr(&peer_addr, "peer_addr")?;
    ...
    config
        ...
        .p2p_socket
        .peers
        .push(crate::P2pSocketPeerConfig {
...
fn apply_profile(config: &mut NodeConfig, profile: ClientProfile) {
    config.reward.reward_source = crate::RewardSourceMode::Tokenomics;
...
fn write_identity_file(
    ...
    let mut password = crate::resolve_identity_password(true)?;
    let store =
...
fn socket_topic_label(topic: crate::P2pTopic) -> &'static str {
    match topic {
        crate::P2pTopic::Observations => "observations",
...
fn build_socket_watch_network(
    ...
    network.bootstrap_peer(
        config.node_id()?,
        &[crate::P2pTopic::Batches, crate::P2pTopic::Receipts],
    )?;
    Ok(network)
}
...
fn sync_reward_game_mappings(config: &mut NodeConfig) {
    ...
    for process_name in &normalized_processes {
        ...
        } else if let Some(mut cached) = load_cached_reward_game_mapping(&cache_path, process_name)
        {
            ...
            let fallback = crate::RewardGameMapping {
