...
- 每区块按 `Annual_Issuance(year) × 区块时间份额 / 年` 铸造年度预算进入模块奖励池，年度预算是铸造硬上限（预算耗尽后该年度不再新增铸造）；
- 奖励申领从年度发行池支付，不再按需无限铸币；
- 销毁闭环：超过 `RewardBurnThreshold` 的奖励申领超额部分按 `RewardBurnBps` 销毁，兑现 `Net Supply Change = Emission − Burn`；
- 年度切换以创世时间为起点、按 365 天协议年推进，跨语言（Rust 链下计算与 Go 链上执行）以同一组 fixtures 锁定公式一致性。
