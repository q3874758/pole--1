...
V1 正式口径在名义发行曲线之上引入**活跃度挂钩调节**：年度实际可发行量不再是名义曲线的机械执行，而是按网络真实活跃度相对锚点的偏离程度动态调整，使发行始终服务于“活跃度是价值基础”这一核心目标。

**公式：**

**Annual_Issuance(year) = Annual_Emission_Amount(year) × Activity_Factor(锚点, 实际活跃度)**

其中：

- `Annual_Emission_Amount(year)` 为名义基准发行量（`总供给 × 名义发行率`，名义曲线见 4.4.1，V1 代码自第 4 年起进入 `2%` 尾部发行）；
- `Activity_Factor = clamp(sqrt(锚点 / 实际活跃度), 1 − cap, 1 + cap)`；
- `锚点 = TargetNetworkWeightUnits`（既有治理参数，单一锚点）；
- `cap = 10%`（协议常量，年度调节上限）；
- `实际活跃度` = 最近一个已结算 epoch 的网络总权重（`TotalNetworkWeightUnits`）。

**语义：**

- 网络实际活跃度低于锚点（网络偏冷）时，`sqrt(锚点/实际)` 大于 1，年度发行上调，最高 `+10%`，用于激励活跃度补足；
- 实际活跃度高于锚点（网络过热）时，年度发行下调，最低 `-10%`，控制过热期新增供给；
- 活跃度与锚点持平或锚点/活跃度数据缺失时，因子为 1.0，发行回到名义曲线。

**链上执行（V1 代码已实现）：**

- 年度预算（名义曲线 × 活跃度因子，±10% cap）按 12 个**月度期**（每期 30 天，协议年 = 360 天）均分，每区块按 `月度预算 × 区块时间份额 / 30 天` 铸造进入模块奖励池，月度预算是铸造硬上限（月度配额耗尽后当月不再新增铸造，次月重置）；
- 奖励申领从月度发行池支付，不再按需无限铸币；
- 销毁闭环：超过 `RewardBurnThreshold` 的奖励申领超额部分按 `RewardBurnBps` 销毁，兑现 `Net Supply Change = Emission − Burn`；
- 月度期与协议年均以创世时间为起点推进，跨语言（Rust 链下计算与 Go 链上执行）以同一组 fixtures 锁定年度预算公式一致性（月度配额 = 年度预算 ÷ 12）。
